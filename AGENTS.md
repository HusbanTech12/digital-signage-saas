...
| ORM | Drizzle ORM | Schema, migrations, type-safe queries |
...
│  (Next.js)       │      │  (REST + Webhooks)│      │  (Drizzle schema)   │
└─────────────────┘      └─────────┬─────────┘      └────────────────────┘
                                    │
                          ┌─────────▼─────────┐
                          │  Redis (pub/sub,   │
...
- Drizzle schema is the single source of truth for DB types — keep it inside `backend/db` and mirror/export the shared TS types the frontend needs via a small `frontend/lib/types` module (kept in sync manually, or via a codegen step, since this is a two-folder layout, not a monorepo workspace)
...
├── frontend/              → Next.js (dashboard, menu designer, display/kiosk client)
│   ├── app/
│   ├── components/
│   ├── lib/
